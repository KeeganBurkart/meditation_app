from __future__ import annotations

from dataclasses import dataclass
from datetime import datetime
from typing import List, Dict, Set, Optional, Any
import sqlite3


@dataclass
class FeedItem:
    """Entry in the social activity feed."""

    item_id: int
    user_id: int
    item_type: str  # 'session', 'comment', 'encouragement'
    message: str
    timestamp: datetime
    target_user_id: Optional[int] = None


class ActivityFeed:
    """Activity feed backed by the database."""

    def __init__(self, conn: Any) -> None:
        self._conn = conn
        self._friends: Dict[int, Set[int]] = {}

    def add_friend(self, user_id: int, friend_id: int) -> None:
        """Establish a friendship so ``user_id`` sees ``friend_id`` in their feed."""
        self._friends.setdefault(user_id, set()).add(friend_id)

    def log_session(self, user_id: int, description: str) -> int:
        """Add a meditation session entry."""
        cur = self._conn.execute(
            "INSERT INTO feed_items (user_id, item_type, message, timestamp, target_user_id) "
            "VALUES (?, ?, ?, ?, ?) RETURNING id",
            (
                user_id,
                "session",
                description,
                datetime.utcnow().isoformat(),
                None,
            ),
        )
        item_id = cur.fetchone()[0]
        self._conn.commit()
        return item_id

    def add_comment(self, user_id: int, target_user_id: int, text: str) -> int:
        """Post a comment directed at ``target_user_id``."""
        cur = self._conn.execute(
            "INSERT INTO feed_items (user_id, item_type, message, timestamp, target_user_id) "
            "VALUES (?, ?, ?, ?, ?) RETURNING id",
            (
                user_id,
                "comment",
                text,
                datetime.utcnow().isoformat(),
                target_user_id,
            ),
        )
        item_id = cur.fetchone()[0]
        self._conn.commit()
        return item_id

    def add_encouragement(self, user_id: int, target_user_id: int, text: str) -> int:
        """Send encouragement to ``target_user_id``."""
        cur = self._conn.execute(
            "INSERT INTO feed_items (user_id, item_type, message, timestamp, target_user_id) "
            "VALUES (?, ?, ?, ?, ?) RETURNING id",
            (
                user_id,
                "encouragement",
                text,
                datetime.utcnow().isoformat(),
                target_user_id,
            ),
        )
        item_id = cur.fetchone()[0]
        self._conn.commit()
        return item_id

    def get_feed(self, user_id: int, limit: int = 10) -> List[FeedItem]:
        """Return recent feed items from the user's friends."""
        friends = self._friends.get(user_id, set())
        if not friends:
            return []

        placeholders = ",".join("?" for _ in friends)
        sql = (
            "SELECT id, user_id, item_type, message, timestamp, target_user_id "
            f"FROM feed_items WHERE user_id IN ({placeholders}) "
            "ORDER BY timestamp DESC, id DESC LIMIT ?"
        )
        cur = self._conn.execute(sql, (*friends, limit))
        rows = cur.fetchall()
        items = [
            FeedItem(
                r[0],
                r[1],
                r[2],
                r[3],
                datetime.fromisoformat(r[4]),
                r[5],
            )
            for r in rows
        ]
        return items
